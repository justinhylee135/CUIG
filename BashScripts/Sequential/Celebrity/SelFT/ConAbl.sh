#!/bin/bash
#SBATCH --job-name=Sequential-Celebrity-SelFT-0.05-ConAbl
...
echo "Sequential Celebrity Unlearning with SelFT 0.05 ConAbl starting..."
...
models_root="${OUTPUT_ROOT}/Sequential/Celebrity/SelFT/0.05/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Sequential/Celebrity/SelFT/0.05/ConAbl/Results"
...
mkdir -p "${REPO_ROOT}/logs/Sequential/Celebrity/SelFT/0.05/ConAbl" "${prompt_dir}" "${prompt_locks_dir}"
...
for celeb in "${unlearn_celebrities[@]}"; do
    ...
    echo "Completed Sequential Celebrity Unlearning with SelFT 0.05 ConAbl through celebrity: ${celeb}"
done

echo "Sequential Celebrity Unlearning with SelFT 0.05 ConAbl finished!"
