#!/bin/bash
#SBATCH --job-name=Sequential-Celebrity-SelFT-0.40-ESD-SDXL
...
echo "Sequential Celebrity Unlearning with SelFT 0.40 ESD-SDXL starting..."
...
models_root="${OUTPUT_ROOT}/Sequential/Celebrity/SelFT/0.40/ESD-SDXL/Models"
results_root="${OUTPUT_ROOT}/Sequential/Celebrity/SelFT/0.40/ESD-SDXL/Results"
...
mkdir -p "${REPO_ROOT}/logs/Sequential/Celebrity/SelFT/0.40/ESD-SDXL" "${prompt_dir}" "${prompt_locks_dir}"
...
for celeb in "${unlearn_celebrities[@]}"; do
    ...
    echo "Completed Sequential Celebrity Unlearning with SelFT 0.40 ESD-SDXL through celebrity: ${celeb}"
done

echo "Sequential Celebrity Unlearning with SelFT 0.40 ESD-SDXL finished!"
