#!/bin/bash
#SBATCH --job-name=Sequential-Celebrity-Base-ESD-SDXL
...
echo "Sequential Celebrity Unlearning with Base ESD-SDXL starting..."
...
models_root="${OUTPUT_ROOT}/Sequential/Celebrity/Base/ESD-SDXL/Models"
results_root="${OUTPUT_ROOT}/Sequential/Celebrity/Base/ESD-SDXL/Results"
...
mkdir -p "${REPO_ROOT}/logs/Sequential/Celebrity/Base/ESD-SDXL" "${prompt_dir}" "${prompt_locks_dir}"
...
for celeb in "${unlearn_celebrities[@]}"; do
    ...
    echo "Completed Sequential Celebrity Unlearning with Base ESD-SDXL through celebrity: ${celeb}"
done

echo "Sequential Celebrity Unlearning with Base ESD-SDXL finished!"
