#!/bin/bash
#SBATCH --job-name=Sequential-Celebrity-Base-ConAbl
...
echo "Sequential Celebrity Unlearning with Base ConAbl starting..."
...
models_root="${OUTPUT_ROOT}/Sequential/Celebrity/Base/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Sequential/Celebrity/Base/ConAbl/Results"
...
mkdir -p "${REPO_ROOT}/logs/Sequential/Celebrity/Base/ConAbl" "${prompt_dir}" "${prompt_locks_dir}"
...
for celeb in "${unlearn_celebrities[@]}"; do
    ...
    echo "Completed Sequential Celebrity Unlearning with Base ConAbl through celebrity: ${celeb}"
done

echo "Sequential Celebrity Unlearning with Base ConAbl finished!"
