...
#SBATCH --job-name=Sequential-Style-SelFT-0.05-ConAbl
...
#SBATCH --output=logs/Sequential/Style/SelFT/0.05/ConAbl_%j.out
#SBATCH --error=logs/Sequential/Style/SelFT/0.05/ConAbl_%j.err

# Script settings
source ~/.bashrc
set -euo pipefail

echo "Sequential Style Unlearning with SelFT 0.05 ConAbl starting..."
...
models_root="${OUTPUT_ROOT}/Sequential/Style/SelFT/0.05/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Sequential/Style/SelFT/0.05/ConAbl/Results"
...
echo "Sequential Style Unlearning with SelFT 0.05 ConAbl finished!"
