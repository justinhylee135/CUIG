...
echo "Independent Style Merge TIES with ConAbl starting..."
...
source_models_root="${OUTPUT_ROOT}/Independent/Style/Base/ConAbl/Models"
models_root="${OUTPUT_ROOT}/Independent/Style/Merge/TIES/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Independent/Style/Merge/TIES/ConAbl/Results"
...
for style in "${merge_styles[@]}"; do
    ...
    echo "Submitting Merge TIES for ConAbl thru${style}: lambda=${lambda_name}, topk=${topk_name}"

    cat > "${job_file}" <<EOF
#!/bin/bash
#SBATCH --account=YOUR_ACCOUNT
#SBATCH --job-name=Independent-Style-Merge-TIES-ConAbl-thru${style}-${experiment_tag}
