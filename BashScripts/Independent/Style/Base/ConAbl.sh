...
echo "Independent Style Unlearning with Base ConAbl starting..."
...
models_root="${OUTPUT_ROOT}/Independent/Style/Base/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Independent/Style/Base/ConAbl/Results"
...
for style in "${unlearn_styles[@]}"; do
    ...
    job_file="$(mktemp "/tmp/conabl_independent_style_${style}_XXXXXX.sh")"

    cat > "${job_file}" <<EOF
#!/bin/bash
#SBATCH --account=YOUR_ACCOUNT
#SBATCH --job-name=Independent-Style-Base-ConAbl-${style}
...
#SBATCH --output=${logs_root}/ConAbl_${style}_%j.out
#SBATCH --error=${logs_root}/ConAbl_${style}_%j.err

# Script Settings
source ~/.bashrc
set -euo pipefail

echo "Independent Style Unlearning with Base ConAbl starting for style: ${style}..."
...
echo "Independent Style Unlearning with Base ConAbl finished!"
