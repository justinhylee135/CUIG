...
echo "Independent Object Unlearning with Base ConAbl starting..."
...
models_root="${OUTPUT_ROOT}/Independent/Object/Base/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Independent/Object/Base/ConAbl/Results"
...
for object in "${unlearn_objects[@]}"; do
    ...
    job_file="$(mktemp "/tmp/conabl_independent_object_${object}_XXXXXX.sh")"

    cat > "${job_file}" <<EOF
#!/bin/bash
#SBATCH --account=YOUR_ACCOUNT
#SBATCH --job-name=Independent-Object-Base-ConAbl-${object}
...
#SBATCH --output=${logs_root}/ConAbl_${object}_%j.out
#SBATCH --error=${logs_root}/ConAbl_${object}_%j.err

# Script Settings
source ~/.bashrc
set -euo pipefail

echo "Independent Object Unlearning with Base ConAbl starting for object: ${object}..."
...
echo "Independent Object Unlearning with Base ConAbl finished!"
