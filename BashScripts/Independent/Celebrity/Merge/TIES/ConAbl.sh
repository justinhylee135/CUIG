...
echo "Independent Celebrity Merge TIES with ConAbl starting..."
...
source_models_root="${OUTPUT_ROOT}/Independent/Celebrity/Base/ConAbl/Models"
models_root="${OUTPUT_ROOT}/Independent/Celebrity/Merge/TIES/ConAbl/Models"
results_root="${OUTPUT_ROOT}/Independent/Celebrity/Merge/TIES/ConAbl/Results"
logs_root="${REPO_ROOT}/logs/Independent/Celebrity/Merge/TIES"
...
for celeb in "${merge_celebrities[@]}"; do
    ...
    job_file="$(mktemp "/tmp/ties_independent_celebrity_thru${celeb}_${experiment_tag}_XXXXXX.sh")"
    ...
    echo "Submitting Merge TIES for ConAbl thru${celeb}: lambda=${lambda_name}, topk=${topk_name}"

    cat > "${job_file}" <<EOF
#!/bin/bash
#SBATCH --account=${CUIG_SLURM_ACCOUNT}
#SBATCH --job-name=Independent-Celebrity-Merge-TIES-ConAbl-thru${celeb}-${experiment_tag}
...
echo "Independent Celebrity TIES merge starting for thru${celeb} with ${experiment_tag}..."
...
echo "Completed TIES merge for thru${celeb} with ${experiment_tag}"
EOF
...
echo "Independent Celebrity Merge TIES submission finished!"
