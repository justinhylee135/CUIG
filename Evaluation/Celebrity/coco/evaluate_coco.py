...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Batch size for CLIP score dataloader.",
    )
    parser.add_argument(
        "--clip_workers",
        type=int,
        default=4,
        help="Number of workers for CLIP score dataloader.",
    )
    parser.add_argument(
        "--fid_batch_size",
        type=int,
        default=32,
        help="Batch size used for computing Inception features.",
    )
    parser.add_argument(
        "--fid_workers",
        type=int,
        default=4,
        help="Number of workers for the FID dataloader.",
    )
    parser.add_argument(
        "--device",
        default=None,
        help="Device to run models on (e.g. 'cuda', 'cuda:0', or 'cpu').",
    )
    return parser
...
def list_image_paths(input_dir: str) -> list[str]:
    """Search input_dir recursively for image files named IMAGEID_instance.ext."""
...
def load_prompts_dataframe(prompts_csv: str) -> pd.DataFrame:
...
def calculate_fid(
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            features = outputs.squeeze(-1).squeeze(-1).cpu().numpy()
...
def calculate_clip_score(
    ...
    with torch.no_grad():
        for images, texts in dataloader:
            ...
            diag_scores = torch.diagonal(logits_per_image, 0).cpu()
            scores.append(diag_scores)

    clip_scores = torch.cat(scores)
    return float(clip_scores.mean().item())
...
def save_results_excel(output_dir: str, fid: float, clip_score: float | None) -> str:
    ...
    results_df = pd.DataFrame({"Metric": ["FID", "CLIP"], "Value": [fid, clip_score]})
...
def evaluate_coco_metrics(
    ...
    df = load_prompts_dataframe(prompts_csv)
    prompt_lookup = build_prompt_lookup(df)
    ...
    results_path = save_results_excel(output_dir=output_dir, fid=fid, clip_score=clip_score)
