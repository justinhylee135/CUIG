    ...
    def __init__(self, input_channel=4):
        ...
        def discriminator_block(in_filters, out_filters, normalization=True):
            ...
            if normalization:
                ...
                layers.append(nn.SyncBatchNorm(out_filters))  # Sync statistics across GPUs
...
class LatentDiscriminator(nn.Module):
    def __init__(self, input_channels=4):
        ...
        self.initial = nn.Conv2d(input_channels, 64, 3, padding=1)
        ...
        self.block4 = self._make_residual_block(512, 512, stride=2)
    ...
    def _make_residual_block(self, in_c, out_c, stride=1):
        return nn.Sequential(
            nn.Conv2d(in_c, out_c, 3, stride, padding=1),
...
def setup_models_and_tokenizer(args, accelerator):
    ...
    if args.discrim_model == 'patchgan':
        input_channel = 4 if not args.discrim_pixel_space else 3
        discriminator = PatchGANDiscriminator(input_channel=input_channel)
    elif args.discrim_model == 'latent':
        discriminator = LatentDiscriminator()
    print(f"Loaded discriminator model: {discriminator.__class__.__name__}")

    # Use base model for reference parameters rather than previous ckpt
    if args.set_original_params_to_base:
        print(f"Setting original parameters dictionary to base model (keys: {args.parameter_group})...")
        args.original_params = {}
        for key in unet.state_dict().keys():
            if args.parameter_group == "cross-attn" and ("attn2.to_k" in key or "attn2.to_v" in key):
                args.original_params[key] = unet.state_dict()[key].detach().clone().requires_grad_(False)
        print(f"\tNumber of keys recorded for args.original_params: {len(args.original_params)}")
    ...
    return tokenizer, text_encoder, vae, unet, discriminator, noise_scheduler, weight_dtype
