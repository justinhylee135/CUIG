...
def multi_step_denoise(noise_scheduler, model, noisy_latents, timesteps, text_emb, num_steps=1):
    """
    Perform multiple denoising steps to amplify concept-specific differences.
    
    Args:
        noise_scheduler: The noise scheduler
        model: The UNet model
        noisy_latents: Batch of noisy latents
        timesteps: Batch of timesteps
        text_emb: Text embeddings for conditioning
        num_steps: Number of denoising steps to perform
    
    Returns:
        Denoised latents after num_steps denoising iterations
    """
    if num_steps == 1:
        # Single step denoising (original behavior)
        denoised_list = []
        for i in range(noisy_latents.shape[0]):
            noise_pred = model(noisy_latents[i:i+1], timesteps[i:i+1], text_emb[i:i+1]).sample
            denoised_output = noise_scheduler.step(
                noise_pred,
                timesteps[i],
                noisy_latents[i:i+1]
            )
            denoised_list.append(denoised_output.pred_original_sample)
        return torch.cat(denoised_list, dim=0)
    
    # Multi-step denoising
    denoised_batch_list = []
    
    for i in range(noisy_latents.shape[0]):
        current_sample = noisy_latents[i:i+1]
        current_timestep = timesteps[i].item()
        
        # Find the position of current timestep in scheduler's timesteps
        scheduler_timesteps = noise_scheduler.timesteps.cpu().numpy()
        try:
            start_idx = (scheduler_timesteps == current_timestep).nonzero()[0][0]
        except:
            # If exact match not found, find closest
            start_idx = torch.abs(noise_scheduler.timesteps - current_timestep).argmin().item()
        
        # Determine end index (don't go below 0)
        end_idx = max(0, start_idx - num_steps + 1)
        
        # Perform multi-step denoising
        for step_idx in range(start_idx, end_idx - 1, -1):
            if step_idx < 0:
                break
                
            t = noise_scheduler.timesteps[step_idx]
            
            # Predict noise at current timestep
            noise_pred = model(
                current_sample, 
                t.unsqueeze(0).to(current_sample.device), 
                text_emb[i:i+1]
            ).sample
            ...
            if step_idx > end_idx:
                # Intermediate steps
                current_sample = noise_scheduler.step(
                    noise_pred, t, current_sample
                ).prev_sample
            else:
                # Last step - get the predicted original sample
                output = noise_scheduler.step(noise_pred, t, current_sample)
                current_sample = output.pred_original_sample
        
        denoised_batch_list.append(current_sample)
    
    return torch.cat(denoised_batch_list, dim=0)
...
def setup_optimizer_and_params(args, unet, discriminator, text_encoder, tokenizer):
    """
    ...
    print(f"\tDiscriminator learning rate: '{args.dlr}'")
    optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=args.dlr, betas=(0.5, 0.999))

    return optimizer, optimizer_D, modifier_token_id, params_to_optimize
...
def prepare_training_state(args, accelerator, unet, frozen_unet, discriminator, text_encoder, optimizer, optimizer_D, train_dataloader, lr_scheduler):
    """
    ...
    if args.parameter_group == "embedding":
        text_encoder, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler = accelerator.prepare(
            text_encoder, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler
        )
        prepared_components = (text_encoder, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler)
    else:
        unet, frozen_unet, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, frozen_unet, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler
        )
        prepared_components = (unet, frozen_unet, optimizer, discriminator, optimizer_D, train_dataloader, lr_scheduler)
...
def print_training_settings_summary(args, accelerator, logger, train_dataloader):
    ...
    logger.info(f"  Discriminator number of warmup steps = {args.discrim_num_warmup_steps}")
    logger.info(f"  Training in pixel space = '{args.discrim_pixel_space}'")
    logger.info(f"  Using random latent instead of anchor latents = '{args.use_random_latent}'")
