...
from src.dynamic_masking import (
    register_attention_hooks as register_dynamic_attention_hooks,
    update_masks_for_unet as update_dynamic_masks_for_unet,
)
...
def main(args):
    ...
    # SculpMem extends ConAbl with dynamic SelFT-style attention masks. Hooks
    # accumulate gradients during warmup, then mask and periodically rotate the
    # trainable attention weights with the largest current gradient signal.
    dynamic_masking_enabled = args.selft_dynamic
    if dynamic_masking_enabled:
        dynamic_update_percent = {
            "q": args.selft_topk,
            "k": args.selft_topk,
            "v": args.selft_topk,
            "to_out": args.selft_topk,
        }
        dynamic_mask_step = 0
        dynamic_mask_warmup = True
        dynamic_turnover_fraction = args.selft_dynamic_initial_turnover_fraction
        dynamic_cross_attention_only = args.parameter_group in ["xattn", "kv-xattn"]

        print("Registering SculpMem dynamic masking hooks...")
        register_dynamic_attention_hooks(
            unet,
            dynamic_update_percent,
            cross_attention_only=dynamic_cross_attention_only,
        )
    else:
        dynamic_update_percent = None
        dynamic_mask_step = 0
        dynamic_mask_warmup = False
        dynamic_turnover_fraction = 0.0
        dynamic_cross_attention_only = True
    ...
    for epoch in range(num_completed_epochs, args.epochs):
        ...
        for iteration_in_epoch, batch in enumerate(anchor_dataloader):
            ...
            with accelerator.accumulate(accumulate_model):
                ...
                # Save the base SculpMem loss before adding regularizer losses.
                # Because next we will combine our loss with our regularizer losses
                sculpmem_loss = loss.detach().clone()
                ...
                if accelerator.sync_gradients:
                    ...
                    if dynamic_masking_enabled:
                        ...
                        if dynamic_mask_warmup and dynamic_mask_step >= args.selft_dynamic_warmup_steps:
                            print("SculpMem dynamic mask warmup finished. Initializing masks...")
                            update_dynamic_masks_for_unet(
                                unet,
                                dynamic_update_percent,
                                dynamic_turnover_fraction,
                                cross_attention_only=dynamic_cross_attention_only,
                            )
                            optimizer.zero_grad()
                            dynamic_mask_warmup = False

                        should_update_dynamic_mask = (
                            args.selft_dynamic_update_mask
                            and not dynamic_mask_warmup
                            and dynamic_mask_step % args.selft_dynamic_mask_update_interval == 0
                            and epoch < int(args.epochs * 0.8)
                        )
                        if should_update_dynamic_mask:
                            if args.selft_dynamic_decay_masks:
                                progress = epoch / max(args.epochs, 1)
                                dynamic_turnover_fraction = (
                                    args.selft_dynamic_initial_turnover_fraction
                                    * 0.5
                            ...
                            print(f"SculpMem dynamic mask update took {time.time() - update_start_time:.4f}s")
            ...
            # If we have regularizers, log the separate loss for just SculpMem
            if l1sp_loss or l2sp_loss: logs["sculpmem_loss"] = sculpmem_loss.item()
