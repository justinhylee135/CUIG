...
def edit_model(
    ...
    projection_max_iters: int = 1,
    projection_convergence_tol: Optional[float] = None,
):
    ...
    for layer_idx in tqdm(
        ...
        with torch.no_grad():
            ...
            base_mat1 = torch.zeros_like(module.weight)
            ...
            erase_contexts: List[torch.Tensor] = []
            retain_contexts: List[torch.Tensor] = []
            ...
            for old_text, new_text in zip(sanitized_old_texts, sanitized_new_texts):
                ...
                erase_contexts.append(old_emb.detach())
            ...
            for retain_text in retain_texts:
                ...
                retain_contexts.append(retain_old.detach())

            identity = torch.eye(module.weight.shape[1], device=weight_device, dtype=weight_dtype)
            ...
            needs_iterative = (
                (proj is not None)
                or (projection_convergence_tol is not None)
                or (max_iters > 1)
            )
            effective_iters = max_iters if needs_iterative else 1
            current_weight = original_weight.clone()
            first_candidate_weight: Optional[torch.Tensor] = None

            iteration_progress = range(effective_iters)
            if needs_iterative:
                iteration_progress = tqdm(
                    iteration_progress,
                    leave=True,
                    desc=f"Editting Layer {layer_idx + 1}/{len(projection_modules)}",
                )

            for iter_idx in iteration_progress:
                total_mat1 = base_mat1 + lamb * current_weight
                total_mat2 = base_mat2 + lambda_identity

                try:
                    solved_weight_t = torch.linalg.solve(total_mat2.T, total_mat1.T)
                    candidate_weight = solved_weight_t.T
                except RuntimeError:
                    candidate_weight = total_mat1 @ torch.linalg.pinv(total_mat2)

                if first_candidate_weight is None:
                    first_candidate_weight = candidate_weight.detach().clone()

                updated_weight = candidate_weight
                if proj is not None:
                    delta = updated_weight - original_weight
                    projected_delta = delta @ proj
                    updated_weight = original_weight + projected_delta

                convergence_gap = None
                if proj is not None and projection_convergence_tol is not None:
                    diffs = []
                    contexts_for_gap = erase_contexts + retain_contexts
                    for context in contexts_for_gap:
                        output_proj = F.linear(context, updated_weight, module.bias)
                        baseline_weight = first_candidate_weight if first_candidate_weight is not None else candidate_weight
                        output_no_proj = F.linear(context, baseline_weight, module.bias)
                        diff = torch.norm(output_proj - output_no_proj, p=2)
                        diffs.append(diff.item())
                    if diffs:
                        convergence_gap = max(diffs)
                        if needs_iterative and hasattr(iteration_progress, "set_postfix"):
                            iteration_progress.set_postfix({"proj_gap": f"{convergence_gap:.3e}", "threshold": f"{projection_convergence_tol:.3e}"})
                    else:
                        convergence_gap = 0.0

                current_weight = updated_weight.detach()

                if (
                    convergence_gap is not None
                    and projection_convergence_tol is not None
                    and convergence_gap <= projection_convergence_tol
                ):
                    break

            module.weight = torch.nn.Parameter(current_weight)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Maximum iterations for projection-constrained refinement",
    )
    parser.add_argument(
        "--projection_convergence_tol",
        type=float,
        default=None,
        help="L2 tolerance between projected and unprojected KV outputs for convergence",
    )
    parser.add_argument(
        "--save_path",
...
def main():
    ...
    ldm_stable = edit_model(
        ...
        projection_convergence_tol=projection_convergence_tol,
    )
